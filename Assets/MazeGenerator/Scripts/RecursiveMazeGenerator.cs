...
public class RecursiveMazeGenerator : BasicMazeGenerator {
	...
	public RecursiveMazeGenerator(int rows, int columns, int coin):base(rows,columns,coin){
		coint = coin;
	}
	...
	private void VisitCell(int row, int column, Direction moveMade){
		...
		do{
			...
			if (coint > 0 && (movesAvailableCount <= 1) && !GetMazeCell(row, column).IsVisited)
			{				
				GetMazeCell(row, column).IsGoal = true;
				coint--;

			} else if (movesAvailableCount == 0 && !GetMazeCell(row, column).IsVisited && !isExit) {
				GetMazeCell(row, column).IsExit = true;
				isExit = true;
			}
