		...
		if (base64Data != null)
		{
			...
			if (bytes.Length > 0)
			{
				...
				{					
					...
					//BluetoothLEHardwareInterface.Log("Check: ValueAction " + deviceAddress + " Characteristic Received: " + characteristic);
#if UNITY_ANDROID
					characteristic = characteristic.ToLower ();
#endif

//#if UNITY_IOS || UNITY_TVOS
//					characteristic = BluetoothLEHardwareInterface.FullUUID(characteristic);
//#endif					
					if (characteristicAction != null && characteristicAction.ContainsKey (characteristic))
					{
						//BluetoothLEHardwareInterface.Log("Check: characteristicValue Characteristic Received: " + characteristic);
						var action = characteristicAction[characteristic];
						//BluetoothLEHardwareInterface.Log("Check: characteristicValue action: " + action);
						if (action != null)
						{
							//BluetoothLEHardwareInterface.Log("Check: value action Characteristic Received: " + characteristic);
				...
				if (DidUpdateCharacteristicValueWithDeviceAddressAction != null && DidUpdateCharacteristicValueWithDeviceAddressAction.ContainsKey (deviceAddress))
				{
					...
					//BluetoothLEHardwareInterface.Log("Check: AddressAction " + deviceAddress + " Characteristic Received: " + characteristic);
#if UNITY_ANDROID
					characteristic = characteristic.ToLower ();
#endif
//#if UNITY_IOS || UNITY_TVOS
...
//#endif					
					if (characteristicAction != null && characteristicAction.ContainsKey (characteristic))
					{
						//BluetoothLEHardwareInterface.Log("Check: characteristicAction Characteristic Received: " + characteristic);
						var action = characteristicAction[characteristic];
						//BluetoothLEHardwareInterface.Log("Check: characteristicAction action: " + action);
						if (action != null)
						{
							//BluetoothLEHardwareInterface.Log("Check: action Characteristic Received: " + characteristic);
